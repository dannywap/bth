...
*/
  public function GetHTML($attributes=null) {
    ...
    if(isset($attributes['allow_create_user']) && $attributes['allow_create_user']){
		$create_user_link="<p class='form-action-link'><a href='".$attributes['create_user_url']."' title='Create a new user account'>Create user</a></p>";
	}
	$elements = $this->GetHTMLForElements();
    $html = <<< EOD
\n<form{$id}{$class}{$name}{$action}{$method}>
<fieldset>
{$elements}{$create_user_link}
</fieldset>
</form>
EOD;
    return $html;
