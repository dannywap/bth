...
function login_menu() {
  ...
  if($ly->user['isAuthenticated']) {
    $gravatar = (get_gravatar(20)!="" ? "<img src='" . get_gravatar(20) . "'>" : null);
    $items = $gravatar."<a href='" . create_url('user/profile') . "'>" . $ly->user['acronym'] . "</a> ";
  ...
  return "<nav class='login_menu'>$items</nav>";
}
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CLydia::Instance()->user['email']))) . '.jpg?d=identicon&amp;' . ($size ? "s=$size" : null);
}
